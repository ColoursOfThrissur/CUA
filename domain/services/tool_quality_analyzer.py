...
class ExecutionLoggerProtocol(Protocol):
    """Minimal contract required from an execution logger."""

    def get_tool_stats(self, tool_name: str, days: int) -> Dict[str, Any]:
        ...

    def get_all_tools_stats(self, days: int) -> Dict[str, Dict[str, Any]]:
        ...


class NullExecutionLogger:
    """Fallback logger used when no infrastructure logger is injected."""
...
class ToolQualityAnalyzer:
    ...
    def __init__(self, execution_logger: ExecutionLoggerProtocol | None = None):
        self.logger = execution_logger or NullExecutionLogger()
