...
class StepResult:
    ...
    def to_serializable(self):
        """
        Convert StepResult to a JSON-serializable dictionary.
        
        Returns:
            dict: JSON-serializable representation of the StepResult.
        """
        return {
            "step_id": self.step_id,
            "status": self.status.value,
            "output": self._make_serializable(self.output),
            "error": self.error,
            "execution_time": self.execution_time,
            "retry_count": self.retry_count,
            "meta": self._make_serializable(self.meta),
...
class ExecutionEngine:
    ...
    def _build_replan_artifacts(self, state: ExecutionState) -> Dict[str, Dict[str, Any]]:
        """Carry forward useful completed outputs so replanning can reuse real artifacts."""
        registry = getattr(state, "state_registry", None)
        if registry is not None:
            registry_artifacts = registry.build_completed_artifacts()
        ...
        for step_id, result in state.step_results.items():
            if result.status != StepStatus.COMPLETED or result.output is None:
                continue
            artifacts[step_id] = self._serialize_replan_output(result.output)
        return artifacts

    def _serialize_replan_output(self, output: Any) -> Dict[str, Any]:
        """Serialize completed step output for replanning prompt carry-forward."""
        try:
            if isinstance(output, (dict, list)):
                text = json.dumps(output, default=str)
                truncated = len(text) > _MAX_REPLAN_TEXT_CHARS
                return {
                    "kind": "structured",
                    "output": text[:_MAX_REPLAN_TEXT_CHARS] + ("..." if truncated else ""),
                    "truncated": truncated,
                }
            text = str(output)
            truncated = len(text) > _MAX_REPLAN_TEXT_CHARS
            ...
            return {
                "kind": "text",
                "output": str(output)[:_MAX_REPLAN_TEXT_CHARS],
                "truncated": True,
            }
    ...
    def _apply_supervisor_decision(
        ...
        if decision.action == DECISION_RETRY_STEP and decision.retry_step_id:
            # Retry the same step locally, optionally swapping to an alternative tool.
            original = next((s for s in all_steps if s.step_id == decision.retry_step_id), None)
            if original:
                retry_tool = decision.alt_tool or original.tool_name
                retry_suffix = (
                    f"retry with {decision.alt_tool}"
                    if decision.alt_tool
                    else "local recovery retry"
                )
                retry_step = TaskStep(
                    step_id=original.step_id,
                    description=f"{original.description} [{retry_suffix}]",
                    ...
                    max_retries=1,
                    preconditions=list(getattr(original, "preconditions", []) or []),
                    postconditions=list(getattr(original, "postconditions", []) or []),
                    checkpoint_policy=getattr(original, "checkpoint_policy", "on_failure"),
                    retry_policy=dict(getattr(original, "retry_policy", {}) or {}),
                )
                if getattr(decision, "wait_seconds", 0.0) > 0:
                    logger.info(
                        f"[SUPERVISOR] Waiting {decision.wait_seconds:.1f}s before retrying {original.step_id}"
                    )
                ...
                state.recovery_attempts[original.step_id] = state.recovery_attempts.get(original.step_id, 0) + 1
                ...
                logger.info(
                    f"[SUPERVISOR] Inserted retry wave for {original.step_id} using {retry_tool}"
                )
            return wave_index + 1
        ...
        if decision.action == DECISION_REPLAN and self.task_planner and remaining_after:
            try:
                ...
                replan_ctx = dict(replan_ctx)
                planner_context = {}
                if getattr(state, "state_registry", None) is not None:
                    planner_context = state.state_registry.build_planner_context()
                if completed_outputs:
                    replan_ctx["completed_summary"] = planner_context.get("completed_summary") or completed_outputs
                    replan_ctx["completed_artifacts"] = planner_context.get("completed_artifacts") or self._build_replan_artifacts(state)
                if planner_context.get("latest_world_state"):
                    replan_ctx["latest_world_state"] = planner_context["latest_world_state"]
                if failed_outputs:
            ...
            try:
                ...
                if self.tool_orchestrator:
                    ...
                    if orchestrated_result.success:
                        recommended_action = str(feedback.get("recommended_action") or "continue")
                        action_status = str(feedback.get("action_status") or "success")
                        if action_status in {"partial", "blocked_waiting", "needs_replan"} and recommended_action in {"retry", "wait_and_retry"}:
                            last_error = (
                                feedback.get("blocking_reason")
                                or orchestrated_result.error
                                or f"{step.tool_name}.{step.operation} reported partial completion"
                            )
                            if attempt < max_retries - 1:
                                wait_seconds = self._get_feedback_wait_seconds(feedback, skill_context)
                                logger.warning(
                                    f"Step {step.step_id} reported {action_status}; waiting {wait_seconds:.1f}s and retrying ({recommended_action})"
                                )
    ...
    def _record_step_state(self, state: ExecutionState, step: TaskStep, result: StepResult) -> None:
        """Persist interpreted step state for recovery and replanning."""
        registry = getattr(state, "state_registry", None)
        if registry is None:
            return
        meta = result.meta or {}
        interpretation = meta.get("result_interpretation") or {}
        registry.record_step(
            ...
            error=result.error,
            artifacts=meta.get("artifacts") or interpretation.get("artifacts"),
            feedback=meta.get("execution_feedback") or interpretation.get("execution_feedback"),
            planner_signal=interpretation.get("planner_signal"),
            resolved_parameters=meta.get("resolved_parameters") or step.parameters,
            execution_time=result.execution_time,
        )

    def _get_feedback_wait_seconds(self, feedback: Dict[str, Any], skill_context: Optional[Any]) -> float:
        """Resolve adaptive wait time from execution feedback."""
        if isinstance(feedback, dict):
            explicit = feedback.get("wait_seconds")
            if explicit is not None:
                try:
                    return max(0.5, float(explicit))
                except Exception:
                    pass
        if skill_context and hasattr(skill_context, "retry_backoff"):
            try:
                return max(0.5, float(skill_context.retry_backoff))
            except Exception:
                pass
        return 2.0
