...
class ToolCreationOrchestrator:
    ...
    def __init__(self, capability_graph, expansion_mode, skill_registry=None, llm_client=None):
        ...
        if skill_registry and llm_client:
            from application.use_cases.tool_lifecycle.tool_creation_skill_aware import SkillAwareCreationOrchestrator
    ...
    def create_new_tool(
        ...
        """
        from infrastructure.logging.tool_creation_logger import get_tool_creation_logger
        ...
        # Load code model for tool creation
        logger.info("Loading code model for tool creation")
        model_manager.load_code_model()
        
        try:
            result = self._create_new_tool_with_code_model(
                gap_description, llm_client, preferred_tool_name, skill_context, creation_logger
            )
            return result
        finally:
            # Always unload code model and restore chat models
            logger.info("Unloading code model, restoring chat models")
            model_manager.unload_code_model()
    
    def _create_new_tool_with_code_model(
        self,
        gap_description: str,
        llm_client,
        ...
        """Inner creation logic with code model loaded."""
    ...
    def _create_new_tool_inner(
        ...
        from infrastructure.code_generation.spec_generator import SpecGenerator
        ...
        from infrastructure.code_generation.tool_creation import QwenCodeGenerator, DefaultCodeGenerator
        ...
        if dep_report.has_missing():
            ...
            if dep_report.missing_services or dep_report.pending_services:
                try:
                    from infrastructure.external.service_generation_integration import ServiceGenerationIntegration
        ...
        from infrastructure.validation.ast.tool_creation_validator import ToolValidator
        ...
        from infrastructure.sandbox.creation_sandbox_runner import SandboxRunner
        from infrastructure.failure_handling.creation_failure_classifier import CreationFailureClassifier
        ...
        if is_stub:
            ...
            try:
                from application.use_cases.evolution.evolution_queue import get_evolution_queue, QueuedEvolution
