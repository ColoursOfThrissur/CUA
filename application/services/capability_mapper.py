...
class CapabilityMapper:
    ...
    def build_capability_graph(self) -> Dict[str, bool]:
        ...
        # Files to exclude from scanning (utilities, not tools)
        EXCLUDED_FILES = {
            'ocr_clicker.py',      # Utility class, not a tool
            'task_state.py',       # Data class for state tracking
            'visual_policy.py',    # Policy functions, no tool class
            'planner_agent.py',    # Agent, not a tool
            'executor_agent.py',   # Agent, not a tool
            'verifier_agent.py',   # Agent, not a tool
