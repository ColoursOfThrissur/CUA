...
class FilesystemTool(BaseTool):
    ...
    def register_capabilities(self):
        ...
        read_capability = ToolCapability(name='read_file', description='Read content from a file', parameters=[Parameter('path', ParameterType.FILE_PATH, 'Path to file to read')], returns='File content as string', safety_level=SafetyLevel.LOW, examples=[{'path': 'output/data.txt'}, {'path': './config.json'}])
        self.add_capability(read_capability, self._handle_read_file)
        write_capability = ToolCapability(name='write_file', description='Write content to a file', parameters=[Parameter('path', ParameterType.FILE_PATH, 'Path to file to write'), Parameter('content', ParameterType.STRING, 'Content to write to file')], returns='Success message with file path', safety_level=SafetyLevel.MEDIUM, examples=[{'path': 'output/result.txt', 'content': 'Hello World'}, {'path': './data.json', 'content': '{"key": "value"}'}])
    ...
    def _handle_list_directory(self, path: str='.') -> List[str]:
        ...
        try:
            ...
            raise NotADirectoryError(f"'{path}' is not a directory") from None
        except OSError as e:
    ...
    def execute(self, operation: str, parameters: Dict[str, Any]):
        ...
        try:
            if operation in self._capabilities:
                ...
                return ToolResult(tool_name=self.__class__.__name__, capability_name=operation, status=ResultStatus.FAILURE, error_message=f"Operation '{operation}' not supported")
        except Exception as e:
            return ToolResult(tool_name=self.__class__.__name__, capability_name=operation, status=ResultStatus.FAILURE, error_message=str(e))
