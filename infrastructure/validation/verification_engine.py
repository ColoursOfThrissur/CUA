...
class VerificationEngine:
    ...
    _SCREEN_OPS = {"capture_screen", "capture_region", "detect_ui_elements", "analyze_screen"}
    ...
    def _layer_heuristic(self, data: Any, tool_name: str, operation: str) -> List[VerificationIssue]:
        ...
        is_screen_op = operation in self._SCREEN_OPS or "screenperceptiontool" in tool_name.lower()
        ...
        if not is_browser_op and not is_web_fetch and not is_screen_op and any(sig in text for sig in _RATE_LIMIT_SIGNALS):
            issues.append(VerificationIssue(
                layer="heuristic", severity="error",
