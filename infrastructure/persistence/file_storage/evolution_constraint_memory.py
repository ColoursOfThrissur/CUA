...
class EvolutionConstraintMemory:
    ...
    def build_constraint_block(self, tool_name: str) -> str:
        ...
        constraints = self._load(tool_name, limit=_MAX_ACTIVE_CONSTRAINTS)
        if not constraints:
            return ""
    ...
    def prune_constraints(
        self,
        tool_name: str,
        max_constraints: int = _MAX_ACTIVE_CONSTRAINTS,
        stale_days: int = _STALE_DAYS,
    ) -> None:
        """Drop stale low-signal constraints and cap the active profile size."""
        try:
            from infrastructure.persistence.sqlite.cua_database import get_conn
            with get_conn() as conn:
                conn.execute(
                    """
                    DELETE FROM evolution_constraints
                    WHERE tool_name=?
                      AND hit_count <= 1
                      AND updated_at < datetime('now', ?)
                    """,
                    (tool_name, f"-{int(stale_days)} days"),
                )
                rows = conn.execute(
                    """
                    SELECT id
                ...
                for row in rows[max_constraints:]:
                    conn.execute("DELETE FROM evolution_constraints WHERE id=?", (row[0],))
        except Exception as e:
            logger.warning(f"Could not prune constraints for {tool_name}: {e}")
    ...
    def _load(self, tool_name: str, limit: Optional[int] = None) -> list:
        try:
            from infrastructure.persistence.sqlite.cua_database import get_conn
            with get_conn() as conn:
                sql = "SELECT type, value FROM evolution_constraints WHERE tool_name=? ORDER BY hit_count DESC, updated_at DESC"
                params = [tool_name]
                if limit:
                    sql += " LIMIT ?"
                    params.append(limit)
