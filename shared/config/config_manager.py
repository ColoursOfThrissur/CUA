...
class LLMConfig(BaseModel):
    ...
    chat_model: str = "qwen"         # For chat, vision, autonomy
    planning_model: str = "mistral"  # For structured JSON planning
    code_model: str = "qwen-coder"   # For tool creation/evolution (on-demand)
    autonomy_model: str = "qwen"     # For autonomy scanning
    
    # Legacy multi-model fields (for backward compatibility)
...
class BrandingConfig(BaseModel):
    internal_project_name: str = "CUA"
    platform_name: str = "Autonomous Agent Platform"
    assistant_name: str = "Platform Assistant"
...
class Config(BaseModel):
    ...
    def load(cls, config_path: str = "config.yaml") -> "Config":
        ...
        if platform_name := os.getenv("CUA_PLATFORM_NAME"):
            config.branding.platform_name = platform_name
        if assistant_name := os.getenv("CUA_ASSISTANT_NAME"):
            config.branding.assistant_name = assistant_name
        if internal_project_name := os.getenv("CUA_INTERNAL_PROJECT_NAME"):
            config.branding.internal_project_name = internal_project_name
