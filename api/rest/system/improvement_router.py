...
def set_loop_instance(loop):
    ...
    loop_instance = loop
    logger.debug("Loop instance set: %s", loop is not None)
...
async def import_plan(request: ImportPlanRequest):
    ...
    try:
        ...
        from domain.policies.immutable_brain_stem import BrainStem
...
@router.post("/improvement/tools/create")
...
@router.get("/improvement/tools/suggest", response_model=ToolSuggestionResponse)
async def suggest_next_tool(skip: int = 0):
    """
    Suggest the next most important tool for Forge to create (self-directed growth),
    ...
    if actionable:
        ...
        try:
            prompt = f"""You are helping {get_platform_name()} decide what tool to create next.
...
"""
    ...
    try:
        ...
        prompt = f"""You are helping {get_platform_name()} (a self-improving agent platform) decide what tool to create next.{skip_hint}
...
"""
    ...
    return ToolSuggestionResponse(
        ...
        description=(
            f"Create a tool that runs a small internal benchmark suite for {get_platform_name()} (a list of tasks/prompts), "
